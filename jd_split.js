...
!(async () => {
  ...
  let res = await getAuthorShareCode('http://adguard.ipq.co/split.json')
  if (!res) {
    $.http.get({url: 'http://adguard.ipq.co/split.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode('http://adguard.ipq.co/split.json')
  }

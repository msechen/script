    ...
    public function index()
    {
        if (Request::isGet()) {
            ...
            $data = $this->_input([
                ...
                'username.require' => 'Login account cannot be empty！',
                'password.require' => 'Login password cannot be empty！',
                'username.min'     => 'The length of the login account cannot be less than 4 valid characters！',
                'password.min'     => 'The length of the login password cannot be less than 4 valid characters！',
            ]);
            if (!CaptchaService::check(input('verify'), input('uniqid'))) {
                $this->error('Graphic verification code verification failed, please re-enter!');
            }
            // 用户信息验证
            $map = ['is_deleted' => '0', 'username' => $data['username']];
            $user = Db::name('SystemUser')->where($map)->order('id desc')->find();
            if (empty($user)) $this->error('Login account or password is wrong, please re-enter!');
            if (md5($user['password'] . session('loginskey')) !== $data['password']) {
                $this->error('Login account or password is wrong, please re-enter!');
            }

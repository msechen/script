...
# wskey 更新cookie
20 8,13,20 * * *  cd /scripts/docker/cookie && node gen_open_token.js >> /scripts/logs/gen_open_token.log 2>&1
...
# 京喜app->领88元红包
5 0,6,18 * * * cd /scripts && ts-node /scripts/jd_jxhb.ts >> /scripts/logs/jd_jxhb.log 2>&1
...
# xxxx
59 11 1 11 * cd /scripts/logs && node /scripts/logs/dreamFactoryProduct.js >> /scripts/logs/dreamFactoryProduct.log 2>&1
...
# 京东-下拉
0 9,13,16,19,23 * * * cd /scripts && ts-node /scripts/jd_homepageTW.ts >> /scripts/logs/jd_homepageTW.log 2>&1

...
if [ ! $CRZAY_CFD_LOOP_ENABLE ]; then
   ...
   echo '' >/scripts/logs/jd_cfd_cashOut.log
   echo '' >/scripts/logs/jd_cfd_loop.log
   cd /scripts && npx tsc /scripts/jd_cfd_cashOut.ts && spnode /scripts/jd_cfd_cashOut.js | ts >>/scripts/logs/jd_cfd_cashOut.log 2>&1 &
   cd /scripts && npx tsc /scripts/jd_cfd_loop.ts && spnode /scripts/jd_cfd_loop.js | ts >>/scripts/logs/jd_cfd_loop.log 2>&1 &

...
install() {
    bash <(curl -Ls https://raw.githubusercontent.com/FranzKafkaYu/x-ui/develop/install.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
update_shell() {
    wget -O /usr/bin/x-ui -N --no-check-certificate https://github.com/FranzKafkaYu/x-ui/raw/develop/x-ui.sh
    if [[ $? != 0 ]]; then
        echo ""

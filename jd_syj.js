...
!(async () => {
  ...
  await getAuthorShareCode('https://gitee.com/DX3242/RandomShareCode/raw/main/JD_Jdzz.json');
  await getAuthorShareCode('https://raw.githubusercontent.com/DX3242/RandomShareCode/main/JD_Jdzz.json');
  await getRandomCode();
  ...
  console.log(`\n\n内部互助 【赚京豆(微信小程序)-瓜分京豆】活动(优先内部账号互助(需内部cookie数量大于${$.assistNum || 4}个)，如有剩余助力次数则给ZFeng3242和随机团助力)\n`)
  for (let i = 0; i < cookiesArr.length; i++) {
    $.canHelp = true
    if (cookiesArr[i]) {
      cookie = cookiesArr[i];
      ...
      if ($.canHelp) {
        ...
        if ($.authorTuanList.length) console.log(`开始账号内部互助 赚京豆-瓜分京豆 活动，如有剩余则给ZFeng3242和随机团助力`)
        for (let j = 0; j < $.authorTuanList.length; ++j) {
          console.log(`账号 ${$.UserName} 开始给ZFeng3242和随机团 ${$.authorTuanList[j]['assistedPinEncrypted']}助力`)

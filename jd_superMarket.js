  ...
  if (smtg_getTeamPkDetailInfoRes && smtg_getTeamPkDetailInfoRes.data.bizCode === 0) {
    ...
    if (joinStatus === 0) {
      if (joinPkTeam === 'true') {
        ...
        await updatePkActivityIdCDN('https://cdn.jsdelivr.net/gh/DX3242/updateTeam@master/shareCodes/jd_updateTeam.json');
...
//新版东东超市
function updatePkActivityId(url = 'https://raw.githubusercontent.com/DX3242/updateTeam/master/jd_updateTeam.json') {
  return new Promise(resolve => {
...
function getTeam() {
  return new Promise(async resolve => {
    $.getTeams = [];
    $.get({url: `https://github.com/DX3242/updateTeam/raw/master/shareCodes/jd_updateTeam.json`, timeout: 100000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(`${JSON.stringify(err)}`)

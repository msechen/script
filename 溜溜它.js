...
async add() {
        try {
            let url = `https://www.chongpar.cn/member/cp-member-integral-award-rule/judge?id=9014`
            let body = ``
            let token = `${this.ck}`
            let urlObject = populateUrlObject(url, token, body)
            await httpRequest('get', urlObject)
            ...
            console.log(`账号[${this.name}] 提现成功了吗 ` + this.brief )
...
async confirm() {
        try {
            ...
            console.log(`账号[${this.name}] 绑定支付宝 已经绑定支付宝`)}else console.log(`账号[${this.name}] `+ `签到奖励` +result.price*0.01 +`元`);
        } catch (e) {
            console.log(e)
        } finally {
            return Promise.resolve(1);

...
//const step = Math.ceil(Math.random() * 10000) + 9300;
...
Object.keys(XMYD_password).forEach((item) => {
  ...
  'User-Agent':'Dalvik/2.1.0 (Linux; U; Android 9; MI 6 MIUI/20.6.18)'
    ...
    $.message +="\n【小米运动改步数提示】：手机、密码数量不匹配或者为填写"+'\n' 
      ...
      console.log(`------------- 共${app_XMYD_phone.length}个账号-------------\n`)
...
async function login(u, p){
  const access_data = await gethttp({
    url: `https://api-user.huami.com/registrations/+86${u}/tokens`, 
    body: {
    "client_id":"HuaMi",
    "password": p,
  ...
  const reg = /(?<=access=).*?(?=&)/;
  const codeArr = reg.exec(access_data.headers.location);
  
  //console.log(codeArr[0]);
  if(!codeArr[0]&&codeArr[0]==''&&codeArr[0]==null){
      console.log(`\n【${$.name}---账号${user}】: 获取 access 失败`)
  }else{
     console.log(`\n【${$.name}---账号${user}】: access 为 ${codeArr[0]}`)
     dl_code=codeArr[0]
  ...
  login_token = access_data.data.token_info.login_token;
  userid = access_data.data.token_info.user_id;
  if(!login_token&&userid){
     console.log(`\n【${$.name}---账号${user}】: 获取用户 token 和 userid 失败`) 
  }else{
     console.log(`\n【${$.name}---账号${user}】: 获取用户 token 为 ${login_token}`)
     console.log(`\n【${$.name}---账号${user}】: 获取用户 userid 为 ${userid}`)
     return {
    login_token,
    userid,}
  }
...
await change_step(app_token, userid)
}
...
function change_step(app_token, user_id) {
  ...
  console.log(`\n【${$.name}---账号${user}】: 当前日期 ${timeFormat(localtime())}`)
      ...
      try {
        if (err) {
          ...
          console.log(`${$.name} API请求失败，请检查网路重试`)
            ...
            if(result.code==1){
                console.log(`\n【${$.name}---账号${user}】: 修改步数结果:${result.message} 步数为:${step}`)
            }else{
              console.log(`\n【${$.name}---账号${user}】: 修改步数结果:${result.message}`)  
            }

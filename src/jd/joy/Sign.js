    ...
    for (const {name, turnTableId, params} of _.filter(_.uniq(signConfig))) {
      if (turnTableId) {
        handleSign(turnTableId, name).then(data => {
          if (!data) return;
          if (!self.isSuccess(data)) return api.log(`[${name}] 签到失败, ${data['errorMessage']}`);
          api.log(`[${name}] 签到成功, 获得 ${_.get(data, 'data.jdBeanQuantity', 0)}`);
        });
      } else if (params) {
        api.commonDo({
          ...
          api.log(`[${name}] 签到成功, ${JSON.stringify(data)}`);
        });
      }
    }

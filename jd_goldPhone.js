...
!(async () => {
    ...
    let res = await getAuthorShareCode('https://raw.githubusercontent.com/DX3242/updateTeam/master/shareCodes/jd_jinji.json')
    if (!res) {
      await $.wait(1000)
      res = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/DX3242/updateTeam@master/shareCodes/jd_jinji.json')
    }
    if (res && res.length > 0) {
        $.tuanId = getRandomArrayElements(res, 1)[0];
        console.log(`ZFeng1005的组队码：${$.tuanId}`);
    }

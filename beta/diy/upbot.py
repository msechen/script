...
async def myupbot(event):
    try:
        ...
        furl = "https://raw.githubusercontent.com/chiupam/JD_Diy/master/shell/diybot_beta.sh"
        ...
        if not resp:
            ...
            return
        cmdtext = f"bash {JD_DIR}/diybot_beta.sh"
        if os.path.exists(f'{BOT_DIR}/diy/user.py'):
            ...
            async with jdbot.conversation(SENDER, timeout=60) as conv:
                ...
                if res == "user":
                    cmdtext = f"bash {JD_DIR}/diybot_beta.sh {res}"
                conv.cancel()

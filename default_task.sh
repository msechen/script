...
if [ $RANDOM_DELAY_MAX ]; then
  if [ $RANDOM_DELAY_MAX -ge 1 ]; then
    ...
    sed -i "/\(jd_bean_sign.js\|jd_blueCoin.js\|jd_joy_reward.js\|jd_joy_steal.js\|jd_joy_feedPets.js\|jd_car_exchange.js\)/!s/\(ts-node\|node\)/sleep \$((RANDOM % \$RANDOM_DELAY_MAX)); node/g" $mergedListFile
  fi
else
...
echo -e "${random_m} ${random_h} * * *  cd /scripts/docker/cookie/gen_open_token.js && >> /scripts/logs/gen_open_token.log 2>&1" | tee -a $mergedListFile
...
echo "第12步启动Node HTTP 更新cookie服务进程……"
pm2 start /scripts/docker/cookie/http.js

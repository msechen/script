...
!(async () => {
  ...
  $.res = await getAuthorShareCode('http://adguard.ipq.co/jxmc-av.json')
  if (!$.res) {
    $.http.get({url: 'http://adguard.ipq.co/jxmc-av.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
    await $.wait(1000)
    $.res = await getAuthorShareCode('http://adguard.ipq.co/jxmc-av.json')
  }
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `http://adguard.ipq.co/jxmc.json`, timeout: 30 * 1000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))

...
let BeansCount = $.getdata('BeansCount') || 0; //兑换多少数量的京豆（200、500或者1000），0表示不兑换，默认不兑换京豆，如需兑换把0改成200、500或者1000即可
...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        ...
        if (cookiesArr[i]) {
            ...
            if ($.isNode()) {
                if (process.env.BeansCount) {
                  BeansCount = process.env.BeansCount;
                }
              }
              try {
                if (BeansCount && Number(BeansCount) !== '0') {
                  await main();
                } else {
    ...
    if ($.token) {
        ...
        if ($.accessToken) {
            ...
            if (!$.hotFlag) {
                if ($.exchangeList) {
                    if($.times == $.limit){
                        console.log(`${$.coins}京豆已达最大兑换数量,请更换所需京豆数量\n`)
                    }else{
                        console.log(`查询兑换${$.exchangeName}ID成功，ID:${$.id}`)
                        await taskPost('do_exchange', `id=${$.id}`);
                    }
                } else {
                    $.log("没有获取到兑换列表！")
                }
                    ...
                    switch (function_id) {
                        case 'get_exchange':
                            ...
                            if (exchange && exchange[0]) {
                                ...
                                $.id = exchange[0].id;
                                $.coins = exchange[0].coins || [];
                                $.limit = exchange[0].times_limit;
                                $.times = exchange[0].exchange_total;
                              }

...
let subTitle = ``;
//let status;
//status = (status = ($.getval("MK_quantity") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let app_soy_kzk_body=[],soy_kzk_body='';
...
!(async () => {
    ...
    if(!$.getdata('soy_kzk_body')&&$.getdata('soy_kzk_body')==''){
        console.log(`\n【${$.name}】：未填写相对应的变量`);
        return;
    }
    
    if($.getdata('soy_kzk_body')&&$.getdata('soy_kzk_body').indexOf('@')>-1){
        soy_kzk_body=$.getdata('soy_kzk_body').split('@');
    }else if($.getdata('soy_kzk_body')&&$.getdata('soy_kzk_body').indexOf('\n')>-1){
        soy_kzk_body=$.getdata('soy_kzk_body').split('\n');
    }else if($.getdata('soy_kzk_body')&&$.getdata('soy_kzk_body').indexOf('&')>-1){
        soy_kzk_body=$.getdata('soy_kzk_body').split('&');
    }else{
        soy_kzk_body=$.getdata('soy_kzk_body').split();
    }
...
console.log(`\n============ 共${app_soy_kzk_body.length}个广告 ============\n`)

//############################
for (i = 0; i < app_soy_kzk_body.length; i++) {
    soy_kzk_body=app_soy_kzk_body[i]

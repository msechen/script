...
const app_soy_sbz_uuid = [],kzkcount = ''
let subTitle = ``;
let status;
status = (status = ($.getval("sbz_status") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
let soy_sbz_uuid = $.getdata('soy_sbz_uuid')
...
appyq = process.env.appyq;//默认邀请,需要自行修改变量
    ...
    if (process.env.soy_sbz_uuid && process.env.soy_sbz_uuid.indexOf('@') > -1) {
        soy_sbz_uuid = process.env.soy_sbz_uuid.split('@');
    } else if (process.env.soy_sbz_uuid && process.env.soy_sbz_uuid.indexOf('\n') > -1) {
        soy_sbz_uuid = process.env.soy_sbz_uuid.split('\n');
    } else if(process.env.soy_sbz_uuid && process.env.soy_sbz_uuid.indexOf('#') > -1){
        soy_sbz_uuid = process.env.soy_sbz_uuid.split('#');
    }else{
        soy_sbz_uuid = process.env.soy_sbz_uuid.split();
    };
    
    Object.keys(soy_sbz_uuid).forEach((item) => {
        if (soy_sbz_uuid[item]) {
            app_soy_sbz_uuid.push(soy_sbz_uuid[item]);
        };
    });
  ...
  app_soy_sbz_uuid.push($.getdata('soy_sbz_uuid'))
...
appyq = $.getdata('appyq');//默认邀请,需要自行修改变量
  ...
  for (let i = 2; i <= kzkcount; i++) {
    app_soy_sbz_uuid.push($.getdata(`soy_sbz_uuid${i}`))
...
for (i = 0; i < app_soy_sbz_uuid.length; i++) {
    ...
    soy_sbz_headers={"Host": "gateway.shangbangzhuan.com",
...
function soy_kzk_lspjl() {
    return new Promise((resolve, reject) => {
        $.get({
            url : `http://gateway.shangbangzhuan.com/task/video/finishWatch`,
            headers : soy_sbz_headers,
        }, async(error, response, data) => {
            //console.log(data)
            let result = JSON.parse(data)

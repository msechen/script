...
!(async () => {
  ...
  let res = await getAuthorShareCode('http://adguard.ipq.co/cfdmoon-av.json')
  if (!res) {
    $.http.get({url: 'http://adguard.ipq.co/cfdmoon-av.json'}).then((resp) => {}).catch((e) => console.log('刷新CDN异常', e));
    await $.wait(1000)
    res = await getAuthorShareCode('http://adguard.ipq.co/cfdmoon-av.json')
  }
  let res2 = await getAuthorShareCode('http://adguard.ipq.co/cfdmoon-zero.json')
  if (!res2) {
    await $.wait(1000)
    res2 = await getAuthorShareCode('http://adguard.ipq.co/cfdmoon-zero.json')
  }
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `http://adguard.ipq.co/cfd.json`, timeout: 10000}, (err, resp, data) => {
      try {
        if (err) {
          console.log(JSON.stringify(err))

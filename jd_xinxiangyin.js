...
!(async () => {
    let res = ['bcae5a8f6c924001a67d31d6b246d049'];
    try{res = await getAuthorShareCode('http://adguard.ipq.co/xinxiangyin.json');}catch (e) {}
    if(!res){
        try{res = await getAuthorShareCode('http://adguard.ipq.co/xinxiangyin.json');}catch (e) {}
        if(!res){res = ['bcae5a8f6c924001a67d31d6b246d049'];}
    }
    if (!cookiesArr[0]) {
...
async function main() {
    ...
    if(!$.activityData.opencard){
        if(!$.shareUuid){$.shareUuid = 'bcae5a8f6c924001a67d31d6b246d049';}
        await join($.venderId);
        await $.wait(2000);
        await takePostRequest('activityContent');
        await $.wait(2000);

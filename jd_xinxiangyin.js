...
!(async () => {
    let res = ['043ba1f933f74109adb82a48df83732a'];
    try{res = await getAuthorShareCode('https://raw.githubusercontent.com/DX3242/RandomShareCode/main/xinxiangyin.json');}catch (e) {}
    if(!res){
        try{res = await getAuthorShareCode('https://cdn.jsdelivr.net/gh/DX3242/RandomShareCode@main/xinxiangyin.json');}catch (e) {}
        if(!res){res = ['043ba1f933f74109adb82a48df83732a'];}
    }
    if (!cookiesArr[0]) {
...
async function main() {
    ...
    if(!$.activityData.opencard){
        if(!$.shareUuid){$.shareUuid = '043ba1f933f74109adb82a48df83732a';}
        await join($.venderId);
        await $.wait(2000);
        await takePostRequest('activityContent');
        await $.wait(2000);
